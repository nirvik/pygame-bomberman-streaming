...
class start(multicast):
	...
	def join_game(self):
		...
		which_player=raw_input('Choose Player \n p1 \n p2 \n p3 \n p4:')#since all the comps are goint to be together,players can discuss amongst themselves and discuss which player no they want 
		id=which_player[-1]
		string=self.uid+' '+id
		...
		for keys in self.players_ids:
			text=font.render("{0} is player {1}".format(keys,self.players_ids[keys]),1,(255,255,255))
